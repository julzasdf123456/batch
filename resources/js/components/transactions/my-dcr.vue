        ...
        <div class="card-body">
          <div class="row">
            ...
            <div class="col-lg-4">
              ...
              <br />
              <button class="btn btn-default ico-tab-mini" @click="fetchPayments()">
                <i class="fas fa-eye ico-tab-mini"></i>View
              </button>
              <button class="btn btn-primary" @click="printDcr()">
                <i class="fas fa-print ico-tab-mini"></i>Print
              </button>
            </div>

            <div v-if="summary.length" class="col-lg-2">
            ...
            <div class="col-lg-6">
              ...
              <h2 class="text-right text-success">
                ₱ <strong>{{ toMoney(cancelledTotal) }}</strong>
              </h2>
            </div>
          </div>
        </div>
          ...
          <ul class="nav nav-tabs" id="custom-tabs-three-tab" role="tablist">
            ...
            <li class="nav-item">
              <a
                ...
                id="cancelled-tab"
                data-toggle="pill"
                href="#cancelled-content"
                role="tab"
                aria-controls="cancelled-content"
                aria-selected="false"
                >Cancelled Payments</a
              >
            </li>
          </ul>
          <!-- TAB BODY -->
                ...
                <table class="table table-sm table-hover table-bordered">
                  ...
                  <tbody>
                    <tr v-for="(item, index) in summary" :key="item.id">
                      ...
                      <td class="v-align">
                        <a
                          v-if="item.TransactionType !== 'Others'"
                          target="_blank"
                          :href="baseURL + '/students/' + item.StudentId"
                      ...
                      <td class="text-right">
                        <div class="btn-group">
                          <button
                            ...
                            class="btn btn-link-muted btn-xs"
                          ...
                          <div class="dropdown" style="display: inline">
                            ...
                            <div class="dropdown-menu" aria-labelledby="more-menu">
                              <button
                                @click="rePrintOR(item.id, item.TransactionType)"
                                class="dropdown-item"
                                title="Reprint OR"
                              >
                                <i class="fas fa-print ico-tab"></i> Re-print OR
                              </button>
                              <button
                                @click="editORNumber(item)"
                                class="dropdown-item"
                                title="Reprint OR"
                              >
                                <i class="fas fa-pen ico-tab"></i> Edit OR Number
                              </button>
                            </div>
                          </div>
                        </div>
                    ...
                    <tr>
                      ...
                      <td class="v-align text-right">
                        <strong>{{ toMoney(summaryTotal) }}</strong>
                      </td>
                      <td></td>
                    </tr>
                  </tbody>
                ...
                <table class="table table-sm table-hover table-bordered">
                  ...
                  <tbody>
                    <tr v-for="(item, index) in details" :key="item.id">
                      ...
                      <td class="v-align text-right">
                        {{ isNull(item.Amount) ? "-" : toMoney(parseFloat(item.Amount)) }}
                      </td>
                      <td class="text-right">
                        <button
                          @click="viewDetails(item.TransactionsId)"
      ...
      <div class="modal-content">
              ...
              <table class="table table-sm table-hover">
                <tbody>
                  ...
                  <tr>
                    ...
                    <td class="v-align text-right">
                      {{ activeTransaction.ModeOfPayment }}
                    </td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    ...
                    <td class="v-align text-right">
                      {{ activeTransaction.CheckAmount }}
                    </td>
                  </tr>
                  <tr>
                    <td class="text-muted v-align">Digital Payment Amount</td>
                    <td class="v-align text-right">
                      {{ activeTransaction.DigitalPaymentAmount }}
                    </td>
                  </tr>
                </tbody>
              </table>
        ...
        <div class="modal-footer">
          <div>
            ...
            <h4 class="no-pads">
              <strong>{{
                toMoney(parseFloat(activeTransaction.TotalAmountPaid))
              }}</strong>
            </h4>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    fetchPayments() {
      // FETCH SUMMARY
      axios
        .get(`${this.baseURL}/transactions/fetch-payments`, {
          params: {
        ...
        .then((response) => {
          ...
          // sum total summary
          this.summaryTotal = this.summary.reduce(
            (sum, item) => sum + parseFloat(item.TotalAmountPaid),
            0
          );
          this.cancelledTotal = this.cancelled.reduce(
            (sum, item) => sum + parseFloat(item.TotalAmountPaid),
            0
          );
        })
      ...
      // FETCH DETAILS
      axios
        .get(`${this.baseURL}/transactions/fetch-all-transaction-details`, {
          params: {
            Date: this.collectionDate,
          },
        })
    ...
    printDcr() {
      window.location.href =
        this.baseURL + "/transactions/print-my-dcr/" + this.collectionDate;
    },
    cancelPayment(id) {
      (async () => {
        const { value: text } = await Swal.fire({
          ...
          title: "Cancel this Payment?",
          text:
            "Before you cancel the payment, please provide a reason for the cancellation.",
          showCancelButton: true,
        });

        if (text) {
          axios
            .post(`${this.baseURL}/transactions/cancel-transaction`, {
              _token: this.token,
              id: id,
              Reason: text,
            })
    ...
    rePrintOR(id, type) {
      if (this.school === "HCA") {
        ...
        if (type === "Tuition Fees") {
          ...
          window.location.href =
            this.baseURL + "/transactions/print-enrollment-svi/" + id;
        } else if (type === "Others") {
          window.location.href =
            this.baseURL + "/transactions/print-other-payments-svi/" + id;
        } else {
          window.location.href =
            this.baseURL + "/transactions/print-miscellaneous-svi/" + id;
        }
      }
    },
    editORNumber(paymentData) {
      (async () => {
        ...
        if (text) {
          axios
            .post(`${this.baseURL}/transactions/update-or-number`, {
              _token: this.token,
              id: paymentData.id,
              NewORNumber: text,
            })
    ...
    orderSummary() {
      const sorted = this.summary.sort((a, b) => {
