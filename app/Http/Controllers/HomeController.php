        ...
        if ($syCurrent != null) {
            $currentDataQuery = DB::table('ClassesRepo')
                ...
                ->select(
                    'Year',
                    'Section',
                    DB::raw("(SELECT COUNT(sc.id) AS Count FROM StudentClasses sc LEFT JOIN Classes c ON sc.ClassId = c.id LEFT JOIN Students s ON s.id=sc.StudentId WHERE c.SchoolYearId='" . $syCurrent->id . "' AND c.Year=ClassesRepo.Year AND c.Section=ClassesRepo.Section AND s.Status IS NULL) AS Count")
                )
                ->orderBy('Year')
                ->orderByRaw("CONCAT(Year, ' - ', Section)")
                ->get()
        ...
        if ($syPrevious != null) {
            $previousDataQuery = DB::table('ClassesRepo')
                ...
                ->select(
                    'Year',
                    'Section',
                    DB::raw("(SELECT COUNT(sc.id) AS Count FROM StudentClasses sc LEFT JOIN Classes c ON sc.ClassId = c.id LEFT JOIN Students s ON s.id=sc.StudentId WHERE c.SchoolYearId='" . $syPrevious->id . "' AND c.Year=ClassesRepo.Year AND c.Section=ClassesRepo.Section AND s.Status IS NULL) AS Count")
                )
                ->orderBy('Year')
                ->orderByRaw("CONCAT(Year, ' - ', Section)")
                ->get()
    ...
    public function getSeniorEnrolleesTrend(Request $request) {
        ...
        if ($syCurrent != null) {
            ...
            $currentDataFirstSemQuery = DB::table('ClassesRepo')
                ->whereRaw("Year IN ('Grade 11', 'Grade 12')")
                ->select(
                    DB::raw("(SELECT COUNT(sc.id) AS Count FROM StudentClasses sc LEFT JOIN Classes c ON sc.ClassId = c.id LEFT JOIN Students s ON s.id=sc.StudentId WHERE c.SchoolYearId='" . $syCurrent->id . "' AND c.Year=ClassesRepo.Year AND c.Section=ClassesRepo.Section AND c.Strand=ClassesRepo.Strand AND c.Semester='1st' AND s.Status IS NULL) AS Count")
                )
                ->groupBy('Year', 'Section', 'Strand')
                ->orderBy('Year')
                ->orderByRaw("CONCAT(Year, ' - ', Strand)")
            ...
            $currentDataSecondSemQuery = DB::table('ClassesRepo')
                ->whereRaw("Year IN ('Grade 11', 'Grade 12')")
                ->select(
                    DB::raw("(SELECT COUNT(sc.id) AS Count FROM StudentClasses sc LEFT JOIN Classes c ON sc.ClassId = c.id LEFT JOIN Students s ON s.id=sc.StudentId WHERE c.SchoolYearId='" . $syCurrent->id . "' AND c.Year=ClassesRepo.Year AND c.Section=ClassesRepo.Section AND c.Strand=ClassesRepo.Strand AND c.Semester='2nd' AND s.Status IS NULL) AS Count")
                )
                ->groupBy('Year', 'Section', 'Strand')
                ->orderBy('Year')
                ->orderByRaw("CONCAT(Year, ' - ', Strand)")
        ...
        if ($syPrevious != null) {
            ...
            $previousDataFirstSemQuery = DB::table('ClassesRepo')
                ...
                ->select(
                    'Year',
                    'Section',
                    DB::raw("(SELECT COUNT(sc.id) AS Count FROM StudentClasses sc LEFT JOIN Classes c ON sc.ClassId = c.id LEFT JOIN Students s ON s.id=sc.StudentId WHERE c.SchoolYearId='" . $syPrevious->id . "' AND c.Year=ClassesRepo.Year AND c.Section=ClassesRepo.Section AND c.Strand=ClassesRepo.Strand AND c.Semester='1st' AND s.Status IS NULL) AS Count")
                )
                ->groupBy('Year', 'Section', 'Strand')
                ->orderBy('Year')
                ->orderByRaw("CONCAT(Year, ' - ', Strand)")
